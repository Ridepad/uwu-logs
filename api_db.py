...
class Cache(DB):
    ...
    def db_was_updated(self):
        if self.on_cooldown():
            # LOGGER.debug(f"=== {self.object_id:40} | {mtime_cached:10} | {mtime_current:10} | Cooldown")
            return False
        ...
        if mtime_current == mtime_cached:
            LOGGER.debug(f"=== {self.object_id:40} | {mtime_cached:10} | {mtime_current:10} | Same")
            return False
        
        LOGGER.debug(f"+++ {self.object_id:40} | {mtime_cached:10} | {mtime_current:10} | Updated")
